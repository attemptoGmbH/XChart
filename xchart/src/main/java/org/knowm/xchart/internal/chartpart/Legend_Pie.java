...
public class Legend_Pie<ST extends Styler, S extends Series> extends Legend_<ST, S> {
	...
	public void doPaint(Graphics2D g) {
		...
		for (S series : map.values()) {
			...
			Color legendColor = series.getFillColor();
			if (series instanceof RadarSeries) { // Quickhack to be able to make the fill color transparent.
				legendColor = ((RadarSeries) series).getMarkerColor();
			}
